...
export interface LiveCardRow {
  ...
  src_id: number | null;
  order: number | null;
  section: 'main' | 'prelims' | 'early_prelims';
  status: 'upcoming' | 'live' | 'final' | 'cancelled';
  weight_class: string | null;
  possible_rounds: number | null;
  referee: string | null;
  a: LiveCorner | null;
  b: LiveCorner | null;
  result: { winner: 'a' | 'b' | null; method: string | null; round: number | null; time: string | null; nc: boolean } | null;
}
...
export interface LiveCurrent {
  src_id: number | null;
  fight_id: number | null;
  phase: 'prefight' | 'round' | 'break' | 'over';
  round: number | null;
  possible_rounds: number | null;
  round_length_sec: number;
  round_started_at: string | null;
  clock_sec: number | null;
  clock_observed_at: number;
  official: boolean;
  stats: { a: LiveStatLine | null; b: LiveStatLine | null } | null;
  rounds: { n: number; a: LiveStatLine | null; b: LiveStatLine | null }[];
  timeline: LiveTimelineEntry[];
}

/** The real-time document pushed over wss://live.ufcalendar.com/v1 and
 * returned by GET /v1/events/{id}/live. */
export interface LiveState {
  v: 1;
  seq: number;
  ts: number;
  hash: string;
  event: { id: number | null; slug: string | null; fmid: number | null; name: string | null; org: string; status: 'upcoming' | 'live' | 'completed'; source: string };
  card: LiveCardRow[];
  current: LiveCurrent | null;
  last: { src_id: number | null; fight_id: number | null; winner: 'a' | 'b' | null; method: string | null; round: number | null; time: string | null } | null;
}
